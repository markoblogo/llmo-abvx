...
export const POST = withErrorHandling(async (req: NextRequest) => {
  ...
  // Update backlink_verified status
  const { error } = await supabase.from("links").update({ backlink_verified: true }).eq("id", linkId)

  if (error) {
    throw new Error(`Failed to verify backlink: ${error.message}`)
  }
